...
ERROR_HEATMAP_BOUNDS = "Please, select the value clicking over a cell in the heatmap"
...
charts_task = [  #0 CLUSTERS BLUES
                ...
                taskChart('1heatmap3_Viridis_disc_marker_15', '1heatmap3_Viridis_disc', '1data3', 0, 15, random.randint(0, 50)),
                taskChart('1heatmap8_Viridis_disc_marker_39', '1heatmap8_Viridis_disc', '0data8', 0, 39, random.randint(0, 50)),
                ...
                # 3 CLUSTER BLUE CONT
                taskChart('3heatmap5_Blues_cont_marker_34', '3heatmap5_Blues_cont', '3data5', 0, 34, 99),
                taskChart('3heatmap6_Blues_disc_marker_27', '3heatmap6_Blues_disc', '3data6', 0, 27, 99),

                # 3 CLUSTER VIRIDIS CONT
                taskChart('3heatmap1_Viridis_cont_marker_6', '3heatmap1_Viridis_cont', '3data1', 0, 6, 99),
                taskChart('3heatmap4_Viridis_disc_marker_40', '3heatmap4_Viridis_disc', '3data4', 0, 40, 99)]
...
def saveAnswersTask(question_id):
    ...
    if 'taskA' in request.form and len(request.form['taskA']) > 0 and 'taskB' in request.form and len(request.form['taskB']) > 0 and 'taskB' in request.form and len(request.form['taskB']) > 0 and 'taskC' in request.form and request.form['taskC'] and len(request.form['taskC']) > 0:
        ...
        if val_number_A >=0 and val_number_A <= 50 and val_number_B >=0 and val_number_B <= 50:
            #FALTA
            #COMPROBAR COORDENADAS DENTRO DEL HEATMAP
            # Compute elapsed time in seconds
            if (xx >= 57 and xx <= 445 and yy >= 32 and yy <= 417):
